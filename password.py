...
#   - 예: 대문자 O와 숫자 0, 소문자 l과 숫자 1, 대문자 I와 숫자 1 등
#   - 이러한 문자들을 제외하면 비밀번호의 가독성이 향상되고 입력 오류가 줄어듭니다
AMBIGUOUS_CHARACTERS = set("O0olI1S5Z2B8G6|")

# CHARACTERS: 비밀번호 생성에 사용되는 최종 문자 집합
#   - 영문 대소문자, 숫자, 특수문자를 포함하되 AMBIGUOUS_CHARACTERS에 포함된 문자는 제외
#   - 이 문자열은 generate_password() 함수에서 무작위 비밀번호를 생성할 때 사용됩니다
#   - string.ascii_letters + string.digits + string.punctuation에서 혼동되는 문자를 제거한 결과입니다
CHARACTERS = ''.join(
